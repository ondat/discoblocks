...
build-drivers: ## Build CSI driver WASIs
	docker run -v $(PWD)/drivers:/go/src tinygo/tinygo:0.23.0 bash -c "(cd /go/src/ebs.csi.aws.com ; go mod tidy && tinygo build -o main.wasm -target wasi --no-debug main.go)"
